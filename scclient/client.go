...
func New(url string, logger logr.Logger) Client {

	return Client{
		url:      url,
		counter:  utils.AtomicCounter{Counter: 0},
		Listener: Init(logger)}
}
...
func (client *Client) registerCallbacks() {
	...
	client.socket.OnTextMessage = func(message string, socket gowebsocket.Socket) {
		...
		if message == "" {
			...
			case parser.EVENT:
				client.logger.Info("Received data for event :: ", eventname)
			...
			case parser.PUBLISH:
				...
				client.logger.Info("Publish event received for channel :: ", channel.Channel)
